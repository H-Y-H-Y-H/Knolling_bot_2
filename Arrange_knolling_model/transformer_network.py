...
if __name__ == "__main__":
    ...
    DATAROOT = "../../knolling_dataset/learning_data_818/"
    ...
    for epoch in range(EPOCHS):
        prin_flag = True
        model.train()
        for input_batch, target_data in train_loader:
            # Zero the gradients
            optimizer.zero_grad()

            input_batch = torch.from_numpy(np.asarray(input_batch, dtype=np.float32)).to(device)
            target_data = torch.from_numpy(np.asarray(target_data, dtype=np.float32)).to(device)
            input_batch = input_batch.transpose(1,0)
            target_data = target_data.transpose(1, 0)

            # zero to False
            target_batch_atten_mask = (target_data == 0).bool()
            target_data.masked_fill_(target_batch_atten_mask, -100)

            # Mask the target:
            mask_point = np.random.randint(0, target_data.size(0) + 1)
            # mask_point = 0
            mask = torch.ones_like(target_data, dtype=torch.bool)
            mask[:mask_point] = False
            input_target_batch = torch.clone(target_data)
            ...
            label_mask = torch.ones_like(target_data, dtype=torch.bool)
            label_mask[:(mask_point+1)] = False
            target_data.masked_fill_(label_mask, -100)
            ...
            # Calculate loss
            loss = model.maskedMSELoss(output_batch, target_data)

...
class random_arrangement():
    
    def __init__(self, lwh_list, gap_item, gap_block,
                 item_odd_prevent, block_odd_prevent, upper_left_max, forced_rotate_box, iteration_time):

        self.lwh_list = lwh_list
        self.num_objects = len(self.lwh_list)
    ...
    def calculate_items(self):
        ...
        for iter in range(self.iteration_time):
            ...
            item_sequence = np.random.choice(len(self.lwh_list), len(self.lwh_list), replace=False)
            ...
            for j in range(len(fac)):
                ...
                if item_min_x + item_min_y < all_item_x + all_item_y:
                    ...
                    best_lwh = np.copy(self.lwh_list)
        ...
        return self.reorder_item(min_xy, best_item_config, best_item_sequence, best_lwh)

    def reorder_item(self, min_xy, best_item_config, item_sequence, best_lwh):
        ...
        item_pos = np.zeros([len(best_lwh), 3])
        item_ori = np.zeros([len(best_lwh), 3])

        start_item_x = np.array([0])
        start_item_y = np.array([0])
        previous_start_item_x = start_item_x
        previous_start_item_y = start_item_y

        for m in range(item_row):
            new_row = best_lwh[item_sequence[m, :]]
        ...
        for n in range(item_column):
            new_column = best_lwh[item_sequence[:, n]]
        ...
        for j in range(item_row):
            for k in range(item_column):
                x_pos = start_item_x[j] + (best_lwh[item_sequence[j][k]][0]) / 2
                y_pos = start_item_y[k] + (best_lwh[item_sequence[j][k]][1]) / 2
                item_pos[item_sequence[j][k]][0] = x_pos
                item_pos[item_sequence[j][k]][1] = y_pos
        # print('this is the shape of item pos', item_pos.shape)
        return item_pos, item_ori

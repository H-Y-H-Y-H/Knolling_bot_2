...
class knolling_main(Arm_env):
    ...
    def knolling(self, manipulator_before=None, new_lwh_list=None, crowded_index=None):

        if self.para_dict['real_operate'] == False:
            if self.para_dict['use_lstm_model'] == True:
                manipulator_before, new_lwh_list, pred_conf, crowded_index, prediction, model_output = self.get_obs()
                self.yolo_pose_model.plot_grasp(manipulator_before, prediction, model_output)
            else:
                manipulator_before, new_lwh_list, pred_conf = self.get_obs()

from ENV.task.LSTMGraspEnv import LSTM_grasp_collection_env
from ENV.robot.LSTMGraspRobot import LSTM_grasp_collection_robot
from ASSET.arrange_model_deploy import *
from ASSET.visual_perception import *
from utils import *

class grasp_collection_main():
    ...
    def __init__(self, para_dict=None, lstm_dict=None, knolling_para=None):
        ...
        self.task = LSTM_grasp_collection_env(para_dict=para_dict, lstm_dict=lstm_dict)
        self.robot = LSTM_grasp_collection_robot(para_dict=para_dict, knolling_para=knolling_para)
    ...
    def get_box_gt(self):
        ...
        self.gt_pos_ori = []
        self.gt_ori_qua = []
        for i in range(len(self.task.boxes_index)):
            p.changeDynamics(self.task.boxes_index[i], -1, lateralFriction=self.para_dict['box_lateral_friction'],
                             contactDamping=self.para_dict['box_contact_damping'],
                             contactStiffness=self.para_dict['box_contact_stiffness'])
            cur_qua = np.asarray(p.getBasePositionAndOrientation(self.task.boxes_index[i])[1])
            cur_ori = np.asarray(p.getEulerFromQuaternion(cur_qua))
            cur_pos = np.asarray(p.getBasePositionAndOrientation(self.task.boxes_index[i])[0])
    ...
    def try_unstack(self, data_source=None, data_tar=None, img_index_start=None):
        ...
        if self.img_per_epoch + img_index_start >= self.task.endnum:
            ...
            if self.para_dict['use_lstm_grasp_model'] == True:
                ...
                with open(self.lstm_dict['grasp_model_path'] + "model_pred_anlysis_rdm.txt", "w") as f:
        ...
        if self.para_dict['use_lstm_grasp_model'] == True:
            manipulator_before, new_lwh_list, pred_cls = self.task.get_obs(epoch=self.img_per_epoch + img_index_start)
        else:
            manipulator_before, new_lwh_list, pred_conf = self.task.get_obs(epoch=self.img_per_epoch + img_index_start)

        if len(manipulator_before) <= 1 or len(self.task.boxes_index) <= 1:
        ...
        for i in range(len(start_end)):
            ...
            if success_grasp_flag == False:
                ...
                for j in range(len(self.task.boxes_index)):
                    ...
                    box_ori = np.asarray(p.getEulerFromQuaternion(p.getBasePositionAndOrientation(self.task.boxes_index[j])[1]))# this is the pos after of the grasped box
                    if np.abs(box_pos[0] - last_pos[0]) < 0.02 and np.abs(box_pos[1] - last_pos[1]) < 0.02 and box_pos[2] > 0.06 and \
                        ...
                        for z in range(len(forbid_range)):
                            if np.abs(box_ori[0] - forbid_range[z]) < 0.2:
                                roll_incline = False
                            if np.abs(box_ori[1] - forbid_range[z]) < 0.2:
                                pitch_incline = False
                            if np.abs(box_ori[2] - forbid_range[z]) < 0.2:
                                yaw_incline = False
        ...
        if self.para_dict['use_lstm_grasp_model'] == True:
            ...
            if np.all(grasp_flag == 0):
                np.savetxt(data_tar + "sim_labels/%012d.txt" % (img_index_start + self.img_per_epoch + self.para_dict['output_offset']), yolo_label, fmt='%.04f')
...
if __name__ == '__main__':
    ...
    para_dict = {'start_num': 0, 'end_num': 200, 'thread': 0, 'output_offset': 00000,
                 ...
                 'use_knolling_model': False, 'use_lstm_grasp_model': True, 'use_yolo_model': True}
    ...
    env = grasp_collection_main(para_dict=para_dict, lstm_dict=lstm_dict)
    os.makedirs(data_root + 'sim_images/', exist_ok=True)
    os.makedirs(data_root + 'sim_labels/', exist_ok=True)
    os.makedirs(data_tar + 'sim_images/', exist_ok=True)
    os.makedirs(data_tar + 'sim_labels/', exist_ok=True)
